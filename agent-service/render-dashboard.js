// ═══════════════════════════════════════════════════════════════════
// render-dashboard.js — dashboard JSON → static HTML (read-only)
// Dùng: node render-dashboard.js [path-to-dashboard.json]
// Ghi ra: ../dashboard/index.html  (GitHub Pages serve từ main root)
// ═══════════════════════════════════════════════════════════════════
...
const adRow = (a, kind) => `<li><b>${esc(a.post_id)}</b> (${esc(a.platform)}) ER ${esc(a.er)}% — ${esc(a.why_worked || a.why_failed || "")} <i>${esc(a.action || "")}</i></li>`;
...
*{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f5f5f7;color:var(--ink);margin:0;padding:20px;max-width:880px;margin:0 auto}
...
${(ap.worked?.length || ap.failed?.length) ? `<div class="card">
  ${ap.worked?.length ? `<b>✅ Worked</b><ul>${ap.worked.map((w) => adRow(w)).join("")}</ul>` : ""}
  ${ap.failed?.length ? `<b>❌ Failed</b><ul>${ap.failed.map((f) => adRow(f)).join("")}</ul>` : ""}
